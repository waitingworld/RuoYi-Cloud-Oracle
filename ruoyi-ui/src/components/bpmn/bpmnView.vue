<template>
  <div ref="bpmnViewContain" style="width: 100%;height: 100%;"/>
</template>
...
export default {
    ...
    loadXml: {
      async handler(n, o) {
        if (n && n !== o && this.loadFinish) {
          if (!this.bpmnViewer) {
            const bpmnViewer = await this.initBpmn(this.$refs.bpmnViewContain)
  ...
  methods: {
    initBpmn(bpmnViewContain) {
      return new Promise(resolve => {
          const bpmnViewer = new BpmnViewer({
            container: bpmnViewContain,
          })
          resolve(bpmnViewer)
        }
      );
